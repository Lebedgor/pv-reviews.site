...
- **Страница `/renew` — контракт с плагином**: License tab генерирует `https://pv-reviews.site/renew?license=<activation_key>` (`pro/class-pvr-tab-license.php`). Страница обязана префиллить инпут (`?license=`, алиас `?key=`) и автоматически запускать lookup. Правки скрипта — в ОБОИХ местах: `pages/renew.php` (inline, то что отдает роутер) и `html/renew.js` (статичная копия). Ловушка: auto-prefill должен выполняться ПОСЛЕ регистрации всех обработчиков — `requestSubmit()` до attach обработчика submit делает нативный сабмит/перезагрузку вместо поиска. Роутер (`router.php`) сохраняет query string при 301 с `/en/*`.
